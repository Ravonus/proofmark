...
async function main() {
  ...
  const doc = await client.document.create.mutate({
    ...
    signers: [
      ...
      { label: "Counterparty", signMethod: "EMAIL_OTP", role: "SIGNER", email: "chad@technomancy.it" },
    ],
  });
  ...
  await client.document.sign.mutate({
    ...
    forensic: {
      ...
      behavioral: (() => {
        ...
        const targets = [
          { id: 1, hash: fnv("bot-name"), descriptor: "synthetic|field:pm-company" },
          { id: 2, hash: fnv("bot-email"), descriptor: "synthetic|field:pm-email" },
