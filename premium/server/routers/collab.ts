...
export const collabRouter = createTRPCRouter({
  ...
  /** Auto-create or return existing active session for a document */
  getOrCreateForDocument: authedProcedure
    .input(
      z.object({
        documentId: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      const existing = await collab.findActiveSessionForDocument(ctx.db, input.documentId);
      if (existing) {
        // Already a participant? Just return it. Otherwise join.
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
        const isParticipant = existing.participants.some(
          (p: { userId: string }) => p.userId === ctx.session.address,
        );
        if (!isParticipant) {
          // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
          await collab.joinSessionById(ctx.db, existing.session.id, ctx.session.address, input.displayName, "editor");
        }
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
        return { sessionId: existing.session.id as string, created: false };
      }
      ...
      return { sessionId: result.session.id as string, created: true };
    }),

  /** Generate an invite link with a specific role for a session */
  createInviteLink: authedProcedure
    .input(
      z.object({
        sessionId: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const collab = await requireCollabForSession(ctx);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      const result = await collab.createInviteLink(ctx.db, {
  ...
  /** Join via an invite link token (includes role) */
  joinViaInvite: authedProcedure
    .input(
      z.object({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const collab = await requireCollabForSession(ctx);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      const result = await collab.joinViaInvite(ctx.db, input.inviteToken, ctx.session.address, input.displayName);
      if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Invite link not found or expired" });
      // eslint-disable-next-line @typescript-eslint/no-unsafe-return
      return result;
    }),
