...
import { UpgradePill } from "../billing/upgrade-banner";
...
const NAV_LINKS = [
	...
	{ href: "/new", label: "Create", icon: LayoutDashboard },
	{ href: "/affiliates", label: "Affiliates", icon: BadgeDollarSign },
...
export function Nav({ badge }: { badge?: { label: string; color?: string } }) {
	...
	const operatorStatusQuery = trpc.account.operatorStatus.useQuery(undefined, {
		enabled: isLoggedIn,
		retry: false,
	});
	const canSeeAdmin = Boolean(
		operatorStatusQuery.data &&
			(operatorStatusQuery.data.isOwner ||
				operatorStatusQuery.data.canManageOthers),
	);
	const navLinks = useMemo(
		() =>
			NAV_LINKS.filter(
				(link) => link.href !== "/admin" || canSeeAdmin,
			),
		[canSeeAdmin],
	);
						...
						<div className="space-y-px p-1.5">
							{navLinks.map((link) => {
								...
								return (
									...
									>
										<Icon
											className={`h-3.5 w-3.5 ${active ? "text-accent" : ""}`}
										/>
										{link.label}
									</Link>
								);
