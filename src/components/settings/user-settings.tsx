...
const AiProviderSettings = dynamic(
	() => import("~/generated/premium/components/ai-provider-settings"),
	{
		ssr: false,
		loading: () => (
			<p className="p-4 text-xs text-muted">Loading AI settings...</p>
		),
	},
);
...
export function UserSettings() {
	...
	const currentWallet =
		statusQuery.data?.currentWallet ?? identity.currentWallet;
	...
	const needsWalletLink = walletLinkMessage
		.toLowerCase()
		.includes("link a wallet");
	...
	if (needsWalletLink) {
		return (
			<FadeIn>
				<GlassCard className="p-6 text-center">
					<p className="text-[12px] text-muted">
						Link a wallet to this account before managing wallet-owned settings.
					</p>
				</GlassCard>
			</FadeIn>
		);
		...
		<div className="flex flex-col gap-4 lg:flex-row">
			...
			<div className="min-w-0 flex-1 space-y-4">
				...
				{activeTab === "integrations" && (
					<WorkspaceSettings section="integrations" />
				)}
			...
			<GlassCard className="space-y-3">
				<div>
					...
					<p className="mt-0.5 text-[11px] text-muted">
						Your wallet identity and deployment status.
					</p>
				</div>
...
function buildPdfStyleSettings(
	themePreset: string,
	fieldSummaryStyle: FieldSummaryStyle,
) {
	return {
		themePreset,
		fieldSummaryStyle,
...
function PdfSection() {
	...
	const defaultTemplate =
		(templatesQuery.data ?? []).find((template) => template.isDefault) ?? null;
	...
	const [fieldSummaryStyle, setFieldSummaryStyle] =
		useState<FieldSummaryStyle>("hybrid");
		...
		if (defaultTemplate) {
			updateTemplate.mutate({
				...
				description:
					...
					"Default PDF settings for newly created documents.",
				settings,
				isDefault: true,
			});
			return;
		...
		<FadeIn>
			<GlassCard className="space-y-4">
				<div>
					...
					<p className="mt-0.5 text-[11px] text-muted">
						Default PDF look for new documents. Override per document during
						creation.
					</p>
				</div>
				...
				<div className="grid gap-2 sm:grid-cols-2">
					...
					<Select
						...
						onChange={(value) =>
							setFieldSummaryStyle(value as FieldSummaryStyle)
						}
						options={[
							{
								value: "hybrid",
...
function FeaturesSection({
	...
	const premiumFeatures =
		featureAccessQuery.data?.featureStates.filter((f) => !f.oss) ?? [];
	const ossFeatures =
		featureAccessQuery.data?.featureStates.filter((f) => f.oss) ?? [];
	const canManage =
		statusQuery.data?.canManageSelf || statusQuery.data?.canManageOthers;
			...
			<FadeIn>
				<GlassCard className="space-y-3">
					<div>
						...
						<p className="mt-0.5 text-[11px] text-muted">
							Always available in every deployment.
						</p>
					</div>
...
function InfoCard({
	...
	return (
		<div className="rounded-md border border-[var(--border)] bg-[var(--bg-inset)] p-3">
			...
			<p
				className={`mt-1.5 text-[12px] font-medium text-primary ${mono ? "break-all font-mono text-[10px]" : ""}`}
			>
				{value}
			</p>
...
function StatusPill({
	...
	return (
		<span
			className={`rounded-xs border px-1.5 py-px text-[8px] font-medium uppercase tracking-[0.1em] ${cls}`}
		>
			{label}
		</span>
	);
