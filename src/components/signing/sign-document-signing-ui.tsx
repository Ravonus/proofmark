					...
					<>
						...
						{requiredFieldsRemaining > 0 && (
							<span className="flex items-center gap-1 text-[10px] text-amber-400">
								<AlertTriangle className="h-3 w-3" /> {requiredFieldsRemaining}{" "}
								remaining
							</span>
						)}
						{allFieldsComplete && (
	...
	return (
		...
		>
			...
			<button
				...
				disabled={
					signing || !address || !chain || !claimToken || !currentSigner
				}
			...
			>
				{signing ? (
					...
					<>
						<PenTool className="h-4 w-4" /> Draw your signature in the document
						first
					</>
				) : !allFieldsComplete && totalMyFields > 0 ? (
					<>
						<AlertTriangle className="h-4 w-4" /> {requiredFieldsRemaining}{" "}
						required field
					...
					<>
						...
						{getRecipientActionLabel(currentRole)} Document
					</>
				)}
			</button>
			<button
...
function getSigningPhase(
	...
	if (totalMyFields > 0 && !allFieldsComplete) {
		if (fieldsFilled === 0)
			return { phase: "review", phaseLabel: "Review document" };
			...
			{steps.map((step, idx) => {
				...
				return (
					<div key={step.key} className="flex items-center gap-1 sm:gap-2">
						{idx > 0 && (
							<div
								className={`hidden h-px w-4 sm:block ${isDone ? "bg-emerald-400/40" : "bg-border"}`}
							/>
						)}
		...
		>
			...
			>
				...
				<div className="flex items-center gap-3 text-xs">
					<span className="font-medium text-secondary">
						Field {Math.min(currentFieldIdx + 1, totalMyFields)} of{" "}
						{totalMyFields}
					</span>
