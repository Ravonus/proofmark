...
import { type DocToken, type InlineField, tokenizeDocument } from "~/lib/document/document-tokens";
import { EditorHistory, type EditorSnapshot } from "~/lib/document/editor-history";
import { trpc } from "~/lib/platform/trpc";
import { useEditorStore } from "~/stores/editor";
import { FieldPicker } from "../fields";
import { AddFieldIndicator, AddSectionModal, DragGhost, MobileFieldPanel } from "./document-editor-overlays";
import { SignersDrawer } from "./document-editor-signers";
import { BlockInsertToolbar, renderTokenElements } from "./document-editor-token-renderer";
import { EditorToolbar } from "./document-editor-toolbar";
import type { EditorResult, PreviewValueMap, SignerDef } from "./document-editor-types";
  ...
  const documentPaper = useMemo(
    () => (
      <div
        ...
        onDragLeave={drag.isDragging ? () => drag.setDropTarget(null) : undefined}
      ...
      >
        ...
        >
          ...
          {!previewMode && (
            <BlockInsertToolbar tokens={tokens} activeSigner={activeSigner} insertTokenAfter={cb.insertTokenAfter} />
          )}
        </div>
      </div>
    ),
    [tokens, fields, previewMode, cb, activeFieldId, addMode, showPanel, activeSigner, signers, drag, setActiveFieldId],
  );
  ...
  return (
    <div className={`flex flex-col ${fullscreen ? "fixed inset-0 z-50 bg-[var(--bg-surface)]" : "h-full"}`}>
      <EditorToolbar
        ...
        onSaveTemplate={onSaveTemplate ? () => onSaveTemplate(cb.buildResult(title)) : undefined}
      ...
      <SignersDrawer
        ...
        onRemoveSigner={(idx) => setSigners((p) => p.filter((_, i) => i !== idx))}
      />
      ...
      />
      <DragGhost isDragging={drag.isDragging} ghostPos={drag.ghostPos} label={drag.dragGhostLabel} />
