...
import { type DocToken, type InlineField, PLACEHOLDERS, tokensToContent } from "~/lib/document/document-tokens";
...
export function moveSectionInTokens(c: DocToken[], hi: number, dir: "up" | "down"): DocToken[] {
  const end = findSectionEnd(c, hi);
  const chunk = c.splice(hi, end - hi);
  const prevHi =
    dir === "up"
      ? (() => {
          for (let i = hi - 1; i >= 0; i--) if (c[i]!.kind === "heading" || c[i]!.kind === "subheading") return i;
          return 0;
        })()
      : (() => {
          const afterHi = hi;
...
function findSectionEnd(tokens: DocToken[], hi: number): number {
  let ei = hi + 1;
  while (ei < tokens.length && tokens[ei]!.kind !== "heading" && tokens[ei]!.kind !== "subheading") ei++;
  return ei;
}
...
export function useEditorCallbacks(opts: EditorCallbacksOpts) {
  const { tokens, setTokens, fields, setFields, previewValues, setPreviewValues, signers, title, fieldCounter } = opts;
  ...
  const insertFieldAfterToken = useCallback(
    ...
    [makeNewField, setActiveFieldId, setAddMode, setTokens, setFields],
  );
  ...
  const removeField = useCallback(
    ...
    [setActiveFieldId, setTokens, setFields, setPreviewValues],
  );
  ...
  const removeToken = useCallback(
    ...
    [setActiveFieldId, setTokens, setFields],
  );
