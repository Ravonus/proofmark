	...
	return (
		<div className={`rounded-xl border p-4 ${TIER_TONES[tone]}`}>
			<div className="flex items-center justify-between">
				...
				<span className="border-current/20 rounded-full border px-1.5 py-0.5 text-[10px]">
					{count}
				</span>
			</div>
...
export function SystemRow({
	...
	const toneClass = tone
		? (SYSTEM_TONES[tone] ?? "text-secondary")
		: "text-secondary";
	return (
...
export function StatusPill({
	...
	return (
		<span
			className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-wide ${PILL_TONES[tone]}`}
		>
			{label}
		</span>
	);
	...
	return (
		<label className="space-y-1">
			...
			<input
				...
				className="bg-surface/50 w-full rounded-xl px-3 py-2 text-sm outline-none ring-1 ring-border focus:ring-accent placeholder:text-muted/40"
			/>
		</label>
	);
}
