...
import { BillingSection } from "./admin-billing";
import {
	BrandingSection,
	TemplatesSection,
	WebhooksSection,
} from "./admin-branding-webhooks";
...
function useAdminGate() {
	...
	if (!identity.isSignedIn) {
		const msg = identity.isLoading
			...
			: "Sign in with email or wallet to access admin.";
...
export function AdminPanel() {
	...
	const visibleTabs = ADMIN_TABS.filter(
		(tab) => !tab.premiumOnly || premiumAvailable,
	);
	const currentWallet =
	...
	return (
		<div className="flex flex-col gap-6 lg:flex-row">
			...
			<div className="min-w-0 flex-1 space-y-6">
				{activeTab === "overview" && (
					<OverviewSection status={statusQuery.data} />
				)}
				{activeTab === "users" && (
					<UsersSection
				...
				{activeTab === "security" && (
					<SecuritySection status={statusQuery.data} />
				)}
				{activeTab === "system" && <SystemSection status={statusQuery.data} />}
				{activeTab === "premium" && (
					<PremiumSection status={statusQuery.data} />
				)}
			</div>
		</div>
	);
...
function AdminGateMessage({
	...
	gate: { message: string; isError?: boolean };
}) {
			...
			<FadeIn>
				<GlassCard className="p-8 text-center">
					...
					<p className="mt-3 text-sm text-red-300">
						Access denied. Admin requires the owner wallet or an account linked to it.
					</p>
				</GlassCard>
			</FadeIn>
		);
	...
	return (
		<FadeIn>
			<GlassCard className="p-8 text-center">
				<p className={gate.isError ? "text-sm text-red-300" : "text-muted"}>
					{gate.message}
				</p>
			</GlassCard>
		</FadeIn>
				...
				{status?.ownerWallet && (
					<GlassCard className="space-y-2">
						...
						<p className="text-xs text-muted">
							{CHAIN_META[status.ownerWallet.chain].label}
						</p>
					</GlassCard>
				)}
			</div>
				...
				<GlassCard className="space-y-4">
					<div>
						...
						<p className="mt-1 text-sm text-muted">
							All {catalog.length} features available in this deployment. Use
							the Users tab to manage per-wallet access.
						</p>
					</div>
										...
										{features.map((f) => (
											...
											>
												...
												<div className="flex shrink-0 items-center gap-2">
													<StatusPill
														tone={f.oss ? "success" : "warning"}
														label={f.oss ? "OSS" : "Premium"}
													/>
