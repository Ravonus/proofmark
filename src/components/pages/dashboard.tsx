	...
	return (
		<FadeIn>
			<GlassCard className="p-6" hover={false}>
				<h3 className="mb-5 text-xs font-semibold uppercase tracking-[0.1em] text-muted">
					Get started
				</h3>
	...
	return (
		<div className="flex gap-3">
			<div className="flex flex-col items-center">
				...
				{!isLast && (
					<div
						className={`my-0.5 h-8 w-px ${step.done ? "bg-[var(--success-30)]" : "bg-[var(--border)]"}`}
					/>
				)}
			</div>
						...
						{paginatedDocs.map((item, i) => (
							...
							>
								{isGroup(item) ? (
									<GroupCard
										group={item}
										isLast={i === paginatedDocs.length - 1}
									/>
								) : (
