...
function buildReminderInput(
  ...
  return {
    cadence,
    channels: signers.some((s) => s.phone?.trim()) ? ["EMAIL", "SMS"] : ["EMAIL"],
  };
}
...
function mapSignerForCreate(signer: SignerRow, proofMode: CreateDocumentInput["proofMode"]): CreateSignerInput {
  return {
    ...
    useAdminWallet: signer.useAdminWallet || undefined,
  };
}
...
function buildAutomationPolicy(mode: "FLAG" | "DENY" | "DISABLED", prepMode: "ALLOW" | "FLAG") {
  const humanSteps = ["signature", "consent", "final_submit", "wallet_auth"] as const;
...
function useTemplateHandlers(s: StateType) {
  const generateFromTemplate = useCallback(
    (template: ContractTemplate, currentSigners: SignerRow[]) => {
      const partyNames = currentSigners.map((sig) => sig.label.trim()).filter(Boolean);
    ...
    s.setSigners(
      template.signers.length > 0
        ? (template.signers.map((sig) => ({
            ...
            tokenGates: (sig.tokenGates ?? null) as CreateSignerInput["tokenGates"],
            fields: sig.fields?.map((f) => ({ ...f, type: f.type })),
          })) as SignerRow[])
        : [emptySigner(), emptySigner()],
    );
    s.setExpiresInDays(template.defaults?.expiresInDays ? String(template.defaults.expiresInDays) : "30");
    s.setReminderCadence((template.defaults?.reminder?.cadence ?? "EVERY_2_DAYS") as ReminderInput["cadence"]);
    s.setProofMode((template.defaults?.proofMode ?? "HYBRID") as CreateDocumentInput["proofMode"]);
    s.setSigningOrder((template.defaults?.signingOrder ?? "parallel") as CreateDocumentInput["signingOrder"]);
    s.setShowTemplates(false);
    s.setShowEditor(true);
  };
  ...
  const handlePdfComplete = (result: {
    ...
    s.setSigners(result.signers.length > 0 ? result.signers : [emptySigner(), emptySigner()]);
    s.setSelectedSavedTemplateId(null);
...
function useSubmitHandlers(
  ...
  const handleSubmit = (result: { title: string; content: string; signers: SignerRow[] }) => {
    ...
    createMutation.mutate(
      createDocumentSchema.parse({
        ...
        signers: result.signers.map((sig) => mapSignerForCreate(sig, s.proofMode)),
        templateId: s.selectedSavedTemplateId || undefined,
        pdfStyleTemplateId:
          s.pdfStyleTemplateId && !s.pdfStyleTemplateId.startsWith("__") ? s.pdfStyleTemplateId : undefined,
        ...
        automationPolicy: buildAutomationPolicy(s.automationReviewMode, s.prepAutomationMode),
      }),
    );
  };

  const handleSaveTemplate = (result: { title: string; content: string; signers: SignerRow[] }) => {
    ...
    saveTemplateMutation.mutate(
      saveTemplateSchema.parse({
        ...
        name,
        title: result.title,
        description: s.selectedSavedTemplateId ? "Updated from editor" : "Saved from editor",
        content: result.content,
...
export function useCreateDocument() {
  ...
  const createMutation = trpc.document.create.useMutation({
    onSuccess: (data) => s.setCreated({ id: data.id, signerLinks: data.signerLinks }),
  });
