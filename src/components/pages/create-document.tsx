		...
		return (
			<div className="mx-auto max-w-2xl">
				<Suspense fallback={<LoadingCard text="Loading PDF tools..." />}>
					<PdfUpload
						onComplete={ctx.handlePdfComplete}
						onCancel={ctx.resetToHome}
					/>
				</Suspense>
			</div>
		);
...
function EditorView({ ctx }: { ctx: ReturnType<typeof useCreateDocument> }) {
	const hasPdf = !!ctx.pdfSource;

	return (
		<div className="fixed inset-0 top-12 z-30 flex bg-[var(--bg-surface)]">
			<div className={hasPdf ? "flex-1 min-w-0" : "flex-1"}>
			...
			{hasPdf && <PdfSidePanel pdfBase64={ctx.pdfSource!} />}
		</div>
	);
}

function PdfSidePanel({ pdfBase64 }: { pdfBase64: string }) {
	...
	if (!show) {
		return (
			<button
				...
				className="absolute right-2 top-14 z-40 rounded-lg border border-[var(--border)] bg-[var(--bg-card)] px-2 py-1 text-[10px] text-muted shadow hover:text-secondary"
			>
				Show PDF
			</button>
		);
	...
	return (
		<div className="hidden w-[420px] shrink-0 flex-col border-l border-[var(--border)] bg-[var(--bg-card)] lg:flex">
			<div className="flex items-center justify-between border-b border-[var(--border)] px-3 py-1.5">
				<span className="text-xs font-medium text-secondary">Original PDF</span>
				<button
					type="button"
			...
			<div className="flex-1 overflow-auto">
				<PdfDocumentViewer source={pdfBytes} className="h-full" />
			</div>
		</div>
	);
}
...
function HomeView({ ctx }: { ctx: ReturnType<typeof useCreateDocument> }) {
	return (
		<div className="mx-auto max-w-2xl space-y-4">
			<UpgradeBanner />
			<CreateDocumentActions
				onUploadPdf={() => ctx.setShowPdfUpload(true)}
				onBlank={ctx.handleSelectBlank}
			/>
			<DeliveryDefaults
...
function TemplateSection({
	...
	ctx: ReturnType<typeof useCreateDocument>;
}) {
	return (
		<FadeIn delay={0.12}>
			<button
			...
			>
				...
				<motion.span
					animate={{ rotate: ctx.showTemplates ? 180 : 0 }}
		...
		<div className="mx-auto max-w-2xl space-y-4">
			<ScaleIn>
				<GlassCard className="p-6 text-center">
					...
					<p className="text-[12px] text-secondary">
						Share each signer&apos;s unique link below.
					</p>
				</GlassCard>
			</ScaleIn>
			<FadeIn delay={0.15}>
				<GlassCard className="space-y-3 p-5">
					...
					<div className="space-y-2">
						{created.signerLinks.map((link, idx) => (
							<div
								key={idx}
								className="rounded-md border border-[var(--border)] bg-[var(--bg-inset)] p-3"
							>
