...
function startSocialPoll(opts: {
  ...
  const { popup, pollRef, field, deps, getFieldValues } = opts;
  if (pollRef.current) clearInterval(pollRef.current);
  const startedAt = Date.now();
  const POLL_TIMEOUT_MS = 5 * 60 * 1000;
  const POPUP_CLOSE_GRACE_MS = 10 * 1000;
  ...
  pollRef.current = setInterval(() => {
    ...
    if (
      ...
      (popupClosedAt !== null && Date.now() - popupClosedAt > POPUP_CLOSE_GRACE_MS)
    ) {
      stopPolling();
      return;
    }
    void (
      ...
      if (!serverVal) return;
      const ok = processSocialVerifyResult(
        serverVal,
        field,
        deps.inlineFields,
        getFieldValues(),
        deps.handleFieldChange,
      );
      if (ok) {
        stopPolling();
  ...
  const triggerSocialVerify = useCallback(
    (field: InlineField) => {
      ...
      startSocialPoll({
        ...
        field,
        deps,
        getFieldValues: () => useSigningStore.getState().fieldValues,
      });
    },
    [deps],
  );
