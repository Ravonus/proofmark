...
export default function VerifyPage() {
  const search = useVerifySearch();
  const doc = search.verifyQuery.data;
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-3xl space-y-8 px-4 py-10">
        <VerifyHeader />
        <VerifySearchForm search={search} />

        {search.searched && search.detectedType && (
          <DetectedTypeFeedback type={search.detectedType} computedHash={search.computedHash} />
        )}

        {search.verifyQuery.isLoading && search.searched && (
        ...
        {search.searched && !search.verifyQuery.isLoading && !doc && <VerifyNotFound />}

        <AnimatePresence>
          {doc && (
            <VerifyResults
              doc={doc}
              securityModeLabel={securityModeLabel}
              hasEncryptedIpfs={hasEncryptedIpfs}
...
function VerifySearchForm({ search }: { search: ReturnType<typeof useVerifySearch> }) {
  return (
    <form onSubmit={search.handleSearch} className="space-y-2">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            onDrop={(e) => {
              ...
              if (f) void search.handleFileDrop(f);
            }}
          />
        </div>
        <button
          type="submit"
          disabled={!search.input.trim()}
      ...
      <div className="flex items-center justify-between px-2">
        ...
        <button
          type="button"
          onClick={() => search.fileInputRef.current?.click()}
...
function VerifyResults({
  ...
  securityModeLabel: string | null;
  hasEncryptedIpfs: boolean;
  encryptedIpfsCid: string | null;
}) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <VerifySignaturesSection signers={doc.signers} />
      ...
      {doc.auditTrail && doc.auditTrail.length > 0 && (
        <VerifyAuditTrail auditTrail={doc.auditTrail} chainValid={doc.auditChainValid} />
      )}
      <VerifyCLI
        ...
        encryptedIpfsCid={encryptedIpfsCid}
      />
      <VerifyForensicReplay docId={doc.id} />
    </motion.div>
  );
}
...
function VerifyStatusBanner({
  ...
  return (
    <div
      className={`rounded-2xl border p-6 text-center ${completed ? "border-emerald-500/20 bg-emerald-500/5" : "border-amber-500/20 bg-amber-500/5"}`}
    >
      ...
      <p className={`text-lg font-semibold ${completed ? "text-emerald-400" : "text-amber-400"}`}>
        {completed ? "Fully Verified & Signed" : "Pending Signatures"}
      </p>
  ...
  return (
    <div className="glass-card space-y-4 rounded-2xl p-6">
      ...
      <div className="grid gap-3">
        ...
        <InfoRow
          icon={
            doc.status === "COMPLETED" ? <CheckCircle className="h-3.5 w-3.5" /> : <Clock className="h-3.5 w-3.5" />
          }
          label="Status"
          value={doc.status}
...
function VerifyCertificate({
  ...
  const completedAt = lastSigned?.signedAt ? new Date(lastSigned.signedAt).toLocaleString() : "--";
  const rows: Array<[string, string, boolean?]> = [
    ["Document Title", doc.title],
    ["Document ID", doc.id, true],
    ["Content Hash (SHA-256)", `${doc.contentHash.slice(0, 16)}...${doc.contentHash.slice(-8)}`, true],
    ...(hasEncryptedIpfs
      ? [["Encrypted Payload CID", `${encryptedIpfsCid!.slice(0, 16)}...`, true] as [string, string, boolean]]
      : []),
    ["Created", new Date(doc.createdAt).toLocaleString()],
    ["Completed", completedAt],
    ["Security Mode", securityModeLabel ?? "SHA-256 only"],
    ["Proof Mode", doc.proofMode ?? "HYBRID"],
  ];
  const identityLevelCls = (level?: string | null) =>
    level === "L2_VERIFIED"
      ...
      <div className="space-y-3 text-xs">
        <div className="grid gap-2">
          {rows.map(([label, value, mono]) => (
            <div key={label} className="flex justify-between border-b border-white/[0.04] py-1.5">
              <span className="text-muted">{label}</span>
              <span className={mono ? "font-mono text-[11px] text-white/60" : "text-white/80"}>{value}</span>
            </div>
          ))}
        </div>
        <div className="pt-2">
                ...
                {doc.signers.map((s, i) => (
                  <tr key={i} className="border-t border-white/[0.04]">
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`rounded-full px-1.5 py-0.5 text-[9px] font-medium ${identityLevelCls(s.identityLevel)}`}
                      >
  ...
  return (
    <div className="glass-card space-y-4 rounded-2xl p-6">
      <h3 className="flex items-center gap-2 text-sm font-medium text-secondary">
        <Terminal className="h-4 w-4" /> Verify via CLI
      </h3>
...
function VerifyForensicReplay({ docId }: { docId: string }) {
  return (
...
const DETECTED_TYPE_LABELS: Record<string, { icon: typeof Hash; text: string }> = {
  hash: { icon: Hash, text: "Detected as SHA-256 hash" },
  cid: { icon: LinkIcon, text: "Detected as IPFS CID" },
  id: { icon: FileText, text: "Detected as document ID" },
  content: { icon: FileText, text: "Hashing content for lookup" },
};

function DetectedTypeFeedback({ type, computedHash }: { type: string; computedHash: string | null }) {
  const entry = DETECTED_TYPE_LABELS[type];
  if (!entry) return null;
  const Icon = entry.icon;
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1 text-center">
      <p className="flex items-center justify-center gap-1.5 text-xs text-muted">
        <Icon className="h-3 w-3" /> {entry.text}
      </p>
...
function VerifySignaturesSection({
  signers,
}: {
  signers: Array<{
    label: string;
  ...
  return (
    <div className="glass-card space-y-4 rounded-2xl p-6">
      <h3 className="flex items-center gap-2 text-sm font-medium text-secondary">
        <PenLine className="h-4 w-4" /> Cryptographic Signatures
      </h3>
        ...
        {signers.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              {s.signedAt && <p className="text-[10px] text-muted">Signed: {new Date(s.signedAt).toLocaleString()}</p>}
              {s.handSignatureHash && (
                <p className="font-mono text-[10px] text-muted">Ink hash: {s.handSignatureHash.slice(0, 16)}...</p>
              )}
            </div>
          );
        })}
    ...
    <div className="glass-card space-y-4 rounded-2xl p-6">
      <div className="flex items-center justify-between">
        ...
        {chainValid !== undefined && (
          <span
            className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${chainValid ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
          >
            {chainValid ? (
              <>
                ...
                <p className="mt-0.5 text-[10px] text-muted">
                  ...
                  {evt.actor && evt.actor !== "system" && (
                    <span className="ml-2 font-mono">
                      {evt.actor.length > 20 ? `${evt.actor.slice(0, 8)}...${evt.actor.slice(-6)}` : evt.actor}
                    </span>
                  )}
