...
function normalizeRequiredUsername(value: unknown): string | null {
...
function getEquivalentSocialFieldIds(
  signerFields: Array<{
    id?: string;
    type?: string;
    label?: string;
  ...
  const reference = signerFields.find((field) => field.id === fieldId);
  if (!reference) return [fieldId];

  const referenceRequiredUsername = normalizeRequiredUsername(reference.settings?.requiredUsername);
  const referenceLabel = normalizeFieldLabel(reference.label);

  const equivalentIds = signerFields
    .filter((field) => field.id && field.type === reference.type)
    .filter((field) => {
      const candidateRequiredUsername = normalizeRequiredUsername(field.settings?.requiredUsername);
      if (referenceRequiredUsername || candidateRequiredUsername) {
        return candidateRequiredUsername === referenceRequiredUsername;
      }
      return normalizeFieldLabel(field.label) === referenceLabel;
    })
    .map((field) => field.id!);

  return equivalentIds.length > 0 ? equivalentIds : [fieldId];
}
...
async function recordVerification(
  ...
  const equivalentFieldIds = getEquivalentSocialFieldIds(signerFields, pending.fieldId);

  const existing = (signer.socialVerifications as Array<typeof verification & { fieldId: string }> | null) ?? [];
  const updated = [
    ...existing.filter((v) => !equivalentFieldIds.includes(v.fieldId)),
    ...equivalentFieldIds.map((fieldId) => ({ ...verification, fieldId })),
  ];
  ...
  const encodedFieldValue = encodeStructuredFieldValue(fieldValue);
  const updatedFieldValues = { ...currentFieldValues };
  for (const fieldId of equivalentFieldIds) {
    updatedFieldValues[fieldId] = encodedFieldValue;
  }
  ...
  try {
    ...
    const { resolveUnifiedRequestIdentity } = await import("~/server/auth/auth-identity");
    ...
    await storeVerificationSession({
      ...
      metadata: {
        ...
        walletAddress: identity.walletSession?.address?.toLowerCase() ?? null,
...
async function handleOAuthCallback(req: NextRequest, code: string, stateParam: string | null): Promise<Response> {
  ...
  try {
    ...
    const errorResponse = await recordVerification(req, pending, userResult.username, userResult.profileId);
...
export async function GET(req: NextRequest) {
  ...
  return handleOAuthCallback(req, code, url.searchParams.get("state"));
}
