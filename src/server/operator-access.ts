...
import { type WalletChain, detectChain, normalizeAddress } from "~/lib/chains";
...
async function loadDbOwner(): Promise<WalletIdentity | null> {
  try {
    ...
    if (!row) return null;
    const chain = (row.ownerChain === "BASE" ? "ETH" : row.ownerChain) as WalletChain;
    return {
...
export type FeatureAccessState = FeatureDescriptor & {
  ...
  source: "oss" | "premium_runtime" | "override_on" | "override_off" | "runtime_unavailable";
};
...
export function resolveWalletIdentity(address: string, chain?: string | null): WalletIdentity {
...
export function getOwnerWalletFromEnv(): WalletIdentity | null {
  ...
  const ownerChain = normalizeWalletChain(env.OWNER_CHAIN) ?? detectChain(ownerAddress);
...
export async function isOwnerWallet(identity: WalletIdentity): Promise<boolean> {
  const owner = await getOwnerWallet();
  if (!owner) return false;
  return owner.chain === identity.chain && owner.address === normalizeAddress(identity.chain, identity.address);
}
...
export async function canManageFeatureTarget(actor: WalletIdentity, _target: WalletIdentity): Promise<boolean> {
...
async function getOverrideRows(db: Db, target: WalletIdentity) {
  try {
    return await db.query.featureOverrides.findMany({
      where: and(eq(featureOverrides.userAddress, target.address), eq(featureOverrides.userChain, target.chain)),
...
export async function getFeatureAccessStates(db: Db, target: WalletIdentity): Promise<FeatureAccessState[]> {
  ...
  return getFeatureCatalog().map((feature) => {
    ...
    const effectiveEnabled = deploymentEnabled && overrideEnabled !== false;
...
export async function isFeatureEnabledForWallet(
  ...
  const access = (await getFeatureAccessStates(db, target)).find((entry) => entry.id === feature.id);
...
export async function requireFeatureForWallet(
  ...
  throw new TRPCError({
    code: "FORBIDDEN",
    message: message ?? `${getFeatureDescriptor(featureId).label} is disabled for this wallet.`,
  });
}
...
export async function saveFeatureOverrides(
  ...
  if (!(await canManageFeatureTarget(actor, target))) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "This wallet is not allowed to manage feature overrides for that user.",
    });
  }
  ...
  try {
    await db.transaction(async (tx) => {
      for (const override of overrides) {
        ...
        await tx
          ...
          .onConflictDoUpdate({
            target: [featureOverrides.userAddress, featureOverrides.userChain, featureOverrides.featureId],
            set: {
              enabled: override.enabled,
              updatedBy: actor.address,
              updatedAt: new Date(),
    ...
    if (isSchemaDriftError(error)) {
      throw new TRPCError({
        ...
        message: "Feature override tables are not ready yet. Run `npm run db:push` and refresh.",
      });
    }
    throw error;
  }
...
export async function listKnownWallets(db: Db) {
  ...
  const seen = new Map<string, WalletIdentity & { lastSeenAt: string | null }>();

  const upsert = (address: string | null, chain?: string | null, seenAt?: Date | null) => {
    if (!address) return;
    ...
    if (!existing || (timestamp && (!existing.lastSeenAt || existing.lastSeenAt < timestamp))) {
  ...
  return Array.from(seen.values()).sort((a, b) => {
    if (!a.lastSeenAt && !b.lastSeenAt) return a.address.localeCompare(b.address);
...
export async function getOperatorStatus(db: Db, actor: WalletIdentity) {
  ...
  const premiumEnabledCount = featureStates.filter((feature) => !feature.oss && feature.effectiveEnabled).length;
...
export async function getFeatureAccessForInput(
  ...
  const target = targetAddress ? resolveWalletIdentity(targetAddress, targetChain) : actor;
  if (!(await canManageFeatureTarget(actor, target))) {
    throw new TRPCError({
      code: "FORBIDDEN",
