...
import type { PdfAnalysisResult, PdfEditResult } from "~/lib/document/pdf-types";
  ...
  return {
    ...
    sections: readRecordArray(source.sections).map(function mapSection(
      ...
      return {
        ...
        lineStart: readNumber(s.lineStart ?? s.line_start),
        lineEnd: readNumber(s.lineEnd ?? s.line_end),
        subsections: readRecordArray(s.subsections).map(mapSection),
      };
    }),
    isFilled: readBoolean(source.isFilled ?? source.is_filled),
    blankFieldCount: readNumber(source.blankFieldCount ?? source.blank_field_count),
    filledFieldCount: readNumber(source.filledFieldCount ?? source.filled_field_count),
    walkthrough: readRecordArray(source.walkthrough).map((w) => ({
      step: readNumber(w.step),
      title: readString(w.title),
      description: readString(w.description),
      action: readString(w.action, "review") as PdfAnalysisResult["walkthrough"][number]["action"],
      target: readNullableString(w.target),
      required: readBoolean(w.required),
    })),
  };
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// PDF Editing — fill fields, create templates, flatten
// ═══════════════════════════════════════════════════════════════════════════════

/** Fill AcroForm fields in an existing PDF. */
export async function fillPdfFields(params: {
  pdfBase64: string;
  fieldValues: Array<{ name: string; value: string }>;
  flattenAfter?: boolean;
}): Promise<PdfEditResult> {
  return post("/api/v1/pdf/fill-fields", {
    pdfBase64: params.pdfBase64,
    fieldValues: params.fieldValues,
    flattenAfter: params.flattenAfter ?? false,
  });
}

/** Create a blank template from a filled PDF (clears field values). */
export async function createPdfTemplate(params: {
  pdfBase64: string;
  fieldsToClear?: string[];
}): Promise<PdfEditResult> {
  return post("/api/v1/pdf/create-template", {
    pdfBase64: params.pdfBase64,
    fieldsToClear: params.fieldsToClear ?? [],
  });
}

/** Flatten a PDF (make all form fields non-editable). */
export async function flattenPdf(pdfBase64: string): Promise<PdfEditResult> {
  return post("/api/v1/pdf/flatten", { pdfBase64 });
}
...
export async function logAuditEvent(params: AuditLogParams): Promise<{ eventId: string; eventHash: string }> {
  const { logAuditEvent: dbLog } = await import("~/server/audit/audit");
  return dbLog(params);
}

export async function getAuditTrail(documentId: string) {
  const { getAuditTrail: dbGet } = await import("~/server/audit/audit");
...
export async function verifyAuditChainByDocId(documentId: string): Promise<{ valid: boolean; brokenAt?: number }> {
  const { verifyAuditChain: dbVerify } = await import("~/server/audit/audit");
  return dbVerify(documentId);
}

export async function computeAuditTrailHash(documentId: string): Promise<string> {
  const { computeAuditTrailHash: dbHash } = await import("~/server/audit/audit");
...
export async function assembleForensicEvidence(input: AssembleForensicInput): Promise<ForensicEvidence> {
  ...
  const { assembleForensicEvidence: assemble } = await import("~/server/forensic/forensic");
