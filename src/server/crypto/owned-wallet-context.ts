...
export async function resolveOwnedAdminAccess(ownedWalletContext: OwnedWalletContext) {
  const actor = ownedWalletContext.activeWallet ?? ownedWalletContext.primaryWallet;
  if (actor && (await isOwnerWallet(actor))) {
    return {
      actor,
      adminWallet: actor,
      hasOwnerAccess: true,
  ...
  return {
    actor,
    adminWallet: linkedOwnerWallet,
    hasOwnerAccess: !!linkedOwnerWallet,
  };
}
