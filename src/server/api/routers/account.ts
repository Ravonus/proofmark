...
async function getOperatorStatusForContext(ctx: Awaited<ReturnType<typeof createTRPCContext>>) {
  ...
  const fallbackActor =
    adminAccess.adminWallet ??
    (await getOwnerWallet()) ?? {
      address: "0x0000000000000000000000000000000000000001",
      chain: "ETH" as const,
    };
  const actor =
    ownedWalletContext.activeWallet ??
    ownedWalletContext.primaryWallet ??
    (isDevAdmin() && ownedWalletContext.identity.authSession ? fallbackActor : null);

  if (!actor) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "Link a wallet to this account before managing operator settings.",
    });
  }
  ...
  return {
    ...status,
    isOwner: status.isOwner || adminAccess.hasOwnerAccess,
    canManageOthers: status.canManageOthers || adminAccess.hasOwnerAccess,
    currentWallet: ownedWalletContext.activeWallet ?? ownedWalletContext.primaryWallet ?? null,
