...
function buildSanitizedSigners(
  ...
  return docSigners.map((s) => {
    const creator = creatorOnlyFields(s, doc.id, opts.isCreator);
    return {
      id: s.id,
      label: s.label,
      address: s.address,
      ...
      isYou: opts.claimToken === s.claimToken,
...
async function resolveTokenGateForViewer(
  ...
  const byAddress = effectiveAddress
    ...
    : undefined;
  const targetSignerForGate = byClaimToken ?? byAddress;
...
export const documentQueryRouter = createTRPCRouter({
  get: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const authorized =
        viewerAccess.canAccessDocument ||
