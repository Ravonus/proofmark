...
/** How a signer authenticates — wallet vs email verification vs manual import. */
export const signMethodEnum = pgEnum("sign_method", [
  ...
  "MANUAL_IMPORT", // Imported from a printed/scanned PDF — no forensics
]);
...
export const documents = pgTable(
  "documents",
  {
    ...
    // ── Hybrid signing: blank PDF version exported for printing ──
    // SHA-256 of the blank PDF generated for this document. Lets us prove a
    // returned scan corresponds to the specific printable version that was issued.
    blankPdfHash: text("blank_pdf_hash"),
  },
  (t) => [
...
export const signers = pgTable(
  "signers",
  {
    ...
    // ── Hybrid signing: where this signature came from ──
    // DIGITAL    = signed in-app (wallet or email OTP) with full forensics
    // MANUAL_PDF = imported from a printed/scanned PDF (no forensic evidence)
    signatureSource: text("signature_source").default("DIGITAL").notNull(),
    // When signatureSource = MANUAL_PDF, the original signed scan is stored
    // and its URL + integrity hash are kept as proof of what was uploaded.
    importedPdfUrl: text("imported_pdf_url"),
    importedPdfHash: text("imported_pdf_hash"),
    importedPdfSize: integer("imported_pdf_size"),
  ...
  (t) => [
    ...
    index("signers_signature_source_idx").on(t.signatureSource),
...
export { billingIntervalEnum, invoiceStatusEnum, subscriptionStatusEnum } from "./schema-enums";
