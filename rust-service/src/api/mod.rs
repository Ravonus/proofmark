//! HTTP API — route configuration only.
//!
//! Each domain has its own submodule with handler functions.
//! Request types live in `types.rs`, error helpers in `error.rs`.
...
pub fn configure(cfg: &mut web::ServiceConfig, pdf_analyze_payload_limit_bytes: usize) {
    cfg.service(
        web::scope("/api/v1")
            ...
            .route("/pq/keygen", web::post().to(pqcrypto::pq_keygen))
            .route("/pq/encrypt", web::post().to(pqcrypto::pq_encrypt))
            .route("/pq/decrypt", web::post().to(pqcrypto::pq_decrypt))
            // Zero-knowledge proofs
            .route("/zk/document-proof", web::post().to(pqcrypto::zk_document_proof))
            .route("/zk/verify-document-proof", web::post().to(pqcrypto::zk_verify_document_proof))
            .route("/zk/signature-proof", web::post().to(pqcrypto::zk_signature_proof))
            .route("/zk/verify-signature-proof", web::post().to(pqcrypto::zk_verify_signature_proof))
            .route("/zk/field-proof", web::post().to(pqcrypto::zk_field_proof))
            .route("/zk/verify-field-proof", web::post().to(pqcrypto::zk_verify_field_proof)),
    );
}
